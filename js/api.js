...
async function getKnockoutMatches() {
  ...
  // Intento principal: traer todos los partidos de la temporada 2026
  ...
  if (res.status === 401 || res.status === 403) {
    throw new Error('API Key inválida o sin permisos. Verifica la configuración en config.js.');
  }
  if (!res.ok) {
    throw new Error(`Error del servidor (${res.status}). Intenta de nuevo más tarde.`);
  }

  const data = await res.json();
  let matches = (data.matches || []).filter(m => stages.includes(m.stage));

  // Fallback: si la temporada 2026 aún no tiene partidos, pedir solo LAST_16
  if (matches.length === 0) {
...
// ── Datos de demostración (sin API Key) ───────────────────────────────────
